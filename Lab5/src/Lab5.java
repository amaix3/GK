...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
	...
	public void Corkscrew(GL2 gl2, int radius, int numberOfTurn)
	{
		gl2.glClear(GL2.GL_COLOR_BUFFER_BIT);
		
		int numberVertices = numberOfTurn * 100;
		double angleIncrement = numberOfTurn * Math.PI / numberVertices;
		float height = 0;
		...
		gl2.glBegin(GL2.GL_POLYGON);
			for(int i = 1; i <= numberVertices; i++ )
			{
				...
				gl2.glColor3f(x/0.6f, (x+z)*z/0.4f, z/0.5f);
				...
				height += numberOfTurn / 1000f;
			}
		gl2.glEnd();
		
		gl2.glTranslated(0, 5, -5);
		glu.gluCylinder(glu.gluNewQuadric(), 1.1, 1.1, 10, 100, 50);
		
		
		gl2.glScaled(1.1, 1.1, 1.1);
		gl2.glRotated(90, 1, 0, 0);
		gl2.glTranslated(0, 8.1, 0);
		gl2.glBegin(GL2.GL_POLYGON);
			for(int i = 1; i <= numberVertices; i++ )
			{
				angle = i * angleIncrement * Math.PI;
				gl2.glColor3f(1.0f, 0.1f, 0.0f);
				
				x = (float) (radius * Math.cos(angle));
				z = (float) (radius * Math.sin(angle));
				
				gl2.glVertex3f(x, 1, z);
			}
		gl2.glEnd();
		
		
		gl2.glTranslated(0, -9.1, 0);
		gl2.glBegin(GL2.GL_POLYGON);
			for(int i = 1; i <= numberVertices; i++ )
			{
				angle = i * angleIncrement * Math.PI;
